 ...
 */
    ...
    public:
        explicit CDKGPendingMessages(size_t _maxMessagesPerNode, int _invType) : invType(_invType), maxMessagesPerNode(
                _maxMessagesPerNode) {};
        ...
        template<typename Message>
        std::vector <std::pair<NodeId, std::shared_ptr < Message>>>
        PopAndDeserializeMessages(size_t
        maxCount)
        {
 ...
 */
    ...
    private:
        ...
        QuorumPhase phase
        GUARDED_BY(cs) {QuorumPhase_Idle};
        int currentHeight
        GUARDED_BY(cs) {-1};
        int quorumHeight
        GUARDED_BY(cs) {-1};
    ...
    public:
        CDKGSessionHandler(const Consensus::LLMQParams &_params, CBLSWorker &_blsWorker,
    ...
    private:
        ...
        void WaitForNextPhase(QuorumPhase curPhase, QuorumPhase nextPhase, const uint256 &expectedQuorumHash,
