...
public:
    ...
    void updateWallet(interfaces::Wallet &wallet, const uint256 &hash, int status, bool showTransaction) {
        ...
        QList<TransactionRecord>::iterator lower = std::lower_bound(cachedWallet.begin(), cachedWallet.end(), hash,
                                                               TxLessThan());
        QList<TransactionRecord>::iterator upper = std::upper_bound(cachedWallet.begin(), cachedWallet.end(), hash,
                                                               TxLessThan());
        ...
        switch (status) {
            ...
            case CT_UPDATED:
                ...
                Q_EMIT
                parent->dataChanged(parent->index(lowerIndex, TransactionTableModel::Status),
                                    parent->index(upperIndex, TransactionTableModel::Status));
                break;
        }
    }

    void updateAddressBook(interfaces::Wallet &wallet, const QString &address, const QString &label, bool isMine,
        ...
        for (auto &rec: cachedWallet) {
            if (rec.strAddress == address2) {
                ...
                Q_EMIT
                parent->dataChanged(parent->index(index, TransactionTableModel::ToAddress),
                                    parent->index(index, TransactionTableModel::ToAddress));
            }
            index++;
        }
    }
    ...
    TransactionRecord *index(interfaces::Wallet &wallet, int numBlocks, int idx) {
        if (idx >= 0 && idx < cachedWallet.size()) {
            ...
            if (rec->statusUpdateNeeded(numBlocks, parent->getChainLockHeight()) &&
                wallet.tryGetTxStatus(rec->hash, wtxStatus, adjustedTime, block_time)) {
                interfaces::WalletTx wtx = wallet.getWalletTx(rec->hash);
                rec->updateStatus(wtx, wtxStatus, numBlocks, adjustedTime, parent->getChainLockHeight(), block_time);
            }
            return rec;
        }
        return nullptr;
 ...
 */
QString
TransactionTableModel::formatAddressLabel(const std::string &address, const QString &label, bool tooltip) const {
    QString description;
    if (!label.isEmpty()) {
        description += label;
    }
    ...
    switch (wtx->type) {
        ...
        case TransactionRecord::FutureSend: {
            if (wtx->label.isEmpty()) {
                return GUIUtil::getThemedQColor(GUIUtil::ThemedColor::SECONDARY);
            }
        }
            break;
        case TransactionRecord::SendToSelf:
    ...
    switch (wtx->status.status) {
        ...
        case TransactionStatus::Confirming:
            switch (wtx->status.depth) {
                case 1:
                    return GUIUtil::getIcon("transaction_1", GUIUtil::ThemedColor::PRIMARY);
                case 2:
                    return GUIUtil::getIcon("transaction_2", GUIUtil::ThemedColor::PRIMARY);
                case 3:
                    return GUIUtil::getIcon("transaction_3", GUIUtil::ThemedColor::PRIMARY);
                case 4:
                    return GUIUtil::getIcon("transaction_4", GUIUtil::ThemedColor::PRIMARY);
                case 5:
                    return GUIUtil::getIcon("transaction_5", GUIUtil::ThemedColor::PRIMARY);
                default:
                    return GUIUtil::getIcon("transaction_6", GUIUtil::ThemedColor::PRIMARY);
            };
        case TransactionStatus::Confirmed:
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const {
    ...
    switch (role) {
        ...
        case LongDescriptionRole:
            return priv->describe(walletModel->node(), walletModel->wallet(), rec,
...
static void
NotifyAddressBookChanged(TransactionTableModel *ttm, const CTxDestination &address, const std::string &label,
...
static void ShowProgress(TransactionTableModel *ttm, const std::string &title, int nProgress) {
    ...
    if (nProgress == 100) {
        ...
        if (vQueueNotifications.size() > 10) { // prevent balloon spam, show maximum 10 balloons
            bool invoked = QMetaObject::invokeMethod(ttm, "setProcessingQueuedTransactions", Qt::QueuedConnection,
                                                     Q_ARG(bool, true));
            assert(invoked);
        }
...
void TransactionTableModel::subscribeToCoreSignals() {
    ...
    m_handler_address_book_changed = walletModel->wallet().handleAddressBookChanged(
