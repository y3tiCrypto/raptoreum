...
SendAssetsDialog::SendAssetsDialog(QWidget *parent) :
    ...
    connect(ui->lineEditAssetControlChange, SIGNAL(textEdited(
    const QString &)), this, SLOT(AssetControlChangeEdited(
    const QString &)));
...
void SendAssetsDialog::setModel(WalletModel *_model) {
    ...
    if (_model && _model->getOptionsModel()) {
        ...
        connect(_model->getOptionsModel(), SIGNAL(coinControlFeaturesChanged(bool)), this,
                SLOT(AssetControlFeatureChanged(bool)));
...
void SendAssetsDialog::send(QList <SendCoinsRecipient> recipients) {
    ...
    for (auto entry: mapAmounts) {
        questionString.append(tr("<b>%1</b><br>")
                                      .arg(BitcoinUnits::formatHtmlWithCustomName(QString::fromStdString(entry.first),
                                                                                  "", entry.second.second,
                                                                                  entry.second.first)));
    }
...
SendAssetsEntry *SendAssetsDialog::addEntry() {
    ...
    connect(entry, SIGNAL(useAvailableAssetsBalance(SendAssetsEntry * )), this,
            SLOT(useAvailableAssetsBalance(SendAssetsEntry * )));
...
void SendAssetsDialog::processSendAssetsReturn(const WalletModel::SendAssetsReturn &sendAssetsReturn,
        ...
        } else // Valid address
        {
            if (!model->wallet().isSpendable(dest)) {
                ...
                if (btnRetVal == QMessageBox::Yes)
                    ...
                    ui->labelAssetControlChangeLabel->setStyleSheet(
